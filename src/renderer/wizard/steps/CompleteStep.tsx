...
export function CompleteStep({ onFinalize, onSaveAsDraft, rdCost }: {
  onFinalize: () => void;
  onSaveAsDraft: () => void;
  rdCost: number;
}) {
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: tokens.spacing.sm, marginBottom: tokens.spacing.sm }}>
          ...
          <span style={{ fontSize: tokens.font.sizeLarge, fontWeight: 600 }}>
            {rdCost === 0 ? "Save Changes" : isEditing ? "Save Redesign" : "Finalize Design"}
          </span>
        </div>
        <p style={{ color: tokens.colors.textMuted, fontSize: tokens.font.sizeBase, margin: 0, marginBottom: tokens.spacing.md }}>
          {rdCost === 0
            ? "No engineering changes detected — save for free. Changing components, screen size, body, ports, or chassis will require R&D."
